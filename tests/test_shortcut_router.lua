...
for _,key in ipairs({'v','c','r'}) do check(not route(event(key))) end
...
check(route(event('t')));check(sent[5]=='NEW_TAB');check(route(event('t',11)))
print('Passed '..n..' selective-shortcut policy checks')

check(route(event('v',10,{cmd=true,shift=true})));check(sent[#sent]=='PASTE_HISTORY');check(route(event('v',11)))
check(route(event('v',10,{cmd=true,alt=true,shift=true})));check(sent[#sent]=='PASTE_MATCH_STYLE');check(route(event('v',11)))
check(not route(event('v',10,{cmd=true})))
check(not route(event('v',10,{cmd=true,ctrl=true,shift=true})))
print('Passed Paste route and ordinary paste isolation checks')
